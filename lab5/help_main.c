...
void Search(struct Data *data) {    //функция поиска файла в директории
    ...
    while ((entry = readdir(dir)) != NULL) {    //считываем файлы из директории
        if (!strcmp(entry->d_name, "Result.txt")) {//если дошли до результирующего файла, то пропускаем его
                continue;
...
void* ThreadWriter(void* fdata) {
	...
	remove("./Files/Result.txt");                  //удаляем файл, если таковой имеется
	op2 = open("./Files/Result.txt", O_WRONLY | O_CREAT);      //открываем/создаем файл с доступом только к записи
...
void CreateThreads(struct Data *data) {         //ф-ия создания потоков
	library_handler = dlopen("./libwriteread.so", RTLD_LAZY);  //получаем указатель на dll(загружаем)
